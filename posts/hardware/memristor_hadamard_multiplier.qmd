...
![Analog in-memory vector matrix multiplication with crossbar array. The parameter matrix is encoded as device conductance. The input vector is encoded as voltage over each column. The current flowing out of each line in parallel carries the output vector. The transistors next to each resistor are used to select the line and column while programming the resistance value.](img/xbar.png){width=70% fig-align="center"}
...
![Multiplier C with input currents Ib colored in green and final output voltages Vy colored in orange. The selector lines Vl are the same signals as in Multiplier A. Activating one selector voltage will direct the current to the corresponding line. We can retrieve the final result vector by reading voltages Vy.](img/memham1/multiplier1c.png){width=70% fig-align="center"}
...
![Multiplier C with input currents Ib colored in green and final output voltages Vy colored in orange. The selector lines $Vl^{[e,r]}$ are the same signals as in Multiplier A. Activating one selector voltage will direct the current to the corresponding line. We can retrieve the final result vector by reading voltages Vy.](img/memham/multiplierc.png){width=70% fig-align="center"}
